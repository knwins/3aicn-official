...
<%
if SaveFile("../en/index.html","http://www.3aicn.com/template/en_index.asp") then 
...
types=rspro("type")
			if SaveFile("../en/"&product_id&".html","http://www.3aicn.com/template/en_newsinfo.asp?id="&product_id&"&types="&types&"") then 
...
do while not rspro.eof
product_id=rspro("product_id")
			if SaveFile("../en/pro_"&product_id&".html","http://www.3aicn.com/template/en_productshow.asp?productid="&product_id&"") then 
...
'===========企业信息生成======'
sqlpro="select * from company where ver='en'"
Set rs_com= Server.CreateObject("ADODB.Recordset")
rs_com.open sqlpro,conn,1,1
do while not rs_com.eof
	id=rs_com("id")
	names=rs_com("name")
	if SaveFile("../en/"&names&".html","http://www.3aicn.com/template/en_company.asp?id="&id&"") then   
    end if 
	rs_com.movenext
	loop
	rs_com.close
...
root_id=rs_root("PRO_CATE_ID")
	...
	do while not rs_big.eof
	   ...
	   if PageCount=0 then PageCount=1 end if
		...
		for k=0 to PageCount
		if SaveFile("../en/probig_"&root_id&"_"&k&".html","http://www.3aicn.com/template/en_product_big.asp?bigtype="&root_id&"&page="&k&"") then
		end if 
		next
		k=k+1
			
	
	if SaveFile("../en/probig_"&root_id&".html","http://www.3aicn.com/template/en_product_big.asp?bigtype="&root_id&"") then   
	end if 
	rs_big.movenext
	loop
	rs_big.close
...
newsname=rs_news_cate("filename")    
	...
	do while not rs_news.eof
	   ...
	   if PageCount=0 then PageCount=1 end if
		...
		for k=1 to PageCount
		if SaveFile("../en/"&newsname&"_"&k&".html","http://www.3aicn.com/template/en_news.asp?type="&news_id&"&page="&k&"") then
		end if 
		next
		k=k+1
			
	
	if SaveFile("../en/"&newsname&".html","http://www.3aicn.com/template/en_news.asp?type="&news_id&"") then   
	end if 
